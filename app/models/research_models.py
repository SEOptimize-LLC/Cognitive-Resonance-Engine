...
class BrandVoice(BaseModel):
    ...
    tone: str = Field(description="e.g., Professional, Friendly, Technical")
    complexity_level: str = Field(description="Simple, Moderate, Technical")
...
class Goal(BaseModel):
    """Customer goal."""
    statement: str
    timeframe: str = Field(description="Immediate or Long-term")
...
class IdealCustomerProfile(BaseModel):
    ...
    icp_name: str
    one_liner: str
    segment_priority: str = Field(description="Primary, Secondary, Niche")
...
class CustomerJob(BaseModel):
    ...
    statement: str
    job_type: str = Field(description="Functional, Social, or Emotional")
...
class Pain(BaseModel):
    ...
    statement: str
    severity: int = Field(ge=1, le=10)
    pain_type: str = Field(description="Functional, Financial, Emotional")


class Gain(BaseModel):
    """Customer gain from VPC."""
    statement: str
    gain_type: str = Field(description="Required, Expected, Desired, Unexpected")
    importance: str
...
class ValuePropositionCanvas(BaseModel):
    ...
    customer_jobs: List[CustomerJob] = Field(default_factory=list)
...
class FinancialPain(BaseModel):
    ...
    statement: str
    category: str  # Direct Cost, Hidden Costs, ROI Uncertainty, etc.
    severity: int = Field(ge=1, le=10)
...
class ForcesAnalysis(BaseModel):
...
class PainPointTaxonomy(BaseModel):
    ...
    functional_pains: List[FunctionalPain] = Field(default_factory=list)
    financial_pains: List[FinancialPain] = Field(default_factory=list)
    emotional_pains: List[EmotionalPain] = Field(default_factory=list)
    
    forces_analysis: Optional[ForcesAnalysis] = None
